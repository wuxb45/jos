...
open(const char *path, int mode)
{
  ...
  if ((path == NULL) || (strnlen(path, MAXPATHLEN) >= MAXPATHLEN)) {
    return -E_BAD_PATH;
  }
  struct Fd *fd = NULL;
  const int ra = fd_alloc(&fd);
  if (ra < 0) {
    return ra;
  }
  //const int rp = sys_page_alloc(0, (void *)fd, PTE_U | PTE_W);
  //if (rp < 0) {
  //  fd_close(fd, 0);
  //  return rp;
  //}
  ...
  const int ro = fsipc(FSREQ_OPEN, fd);
  if (ro < 0) {
    fd_close(fd, 0);
    return ro;
  }
...
devfile_write(struct Fd *fd, const void *buf, size_t n)
{
  ...
  const size_t min = MIN(PGSIZE - sizeof(int) - sizeof(size_t), n);
  fsipcbuf.write.req_n = min;
  memmove(fsipcbuf.write.req_buf, buf, min);
