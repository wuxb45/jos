...
// Yields: npages, npages_basemem, npages_extmem
static void
...
boot_alloc(uint32_t n)
{
  ...
  if (n){
    nalign = ((n - 1) / PGSIZE) * PGSIZE + PGSIZE;
  } else {
    nalign = 0;
  }
  result = nextfree;
  const uint32_t limit = KERNBASE + (npages * PGSIZE);
  if (((uint32_t)result) >= limit) {
...
mem_init(void)
{
  ...
  envs = boot_alloc(NENV * (sizeof(struct Env)));
	...
	// Your code goes here:
  const size_t pages_size = ROUNDUP(npages * sizeof(struct Page), PGSIZE);
  boot_map_region(kern_pgdir, UPAGES, pages_size, PADDR(pages), PTE_U);
	...
	// LAB 3: Your code here.
  const size_t envs_size = ROUNDUP(NENV * (sizeof(struct Env)), PGSIZE);
...
page_init(void)
{
  ...
  // skip [IO ~ ext)
  // ext memory, skip {ext-begin=kernel-start -- kernel-end -- alloced}
  uint32_t ext_free = (uint32_t)boot_alloc(0); // get the next_free
  const size_t extstart = (ext_free - KERNBASE) / PGSIZE;
  for (i = extstart; i < npages; i++) {
    page_free(&pages[i]);
  }
}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  while (PTE_ADDR(p0) <= plast) {
    ...
    pte_t *ppte = pgdir_walk(env->env_pgdir, (void *)p0, 0);
    if (ppte == NULL) {
      goto mem_check_error;
    }
    if (((*ppte) & perm) != perm) {
